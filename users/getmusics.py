...
def getMusics(forroset, mp3folder):
    ...
    for i in range(len(undownloaded)):
        try:
            r = requests.get(
                forroset.loc[ids == undownloaded[i]].preview_url.tolist()[0])
            sleep(0.5)
    ...
    for music in ids:
        if music not in downloaded_musics:
            undownloaded_dict[music] = forroset.loc[ids == music].preview_url.tolist()[
                0]
    ...
    for key in undownloaded_dict.keys():
        c += 1
        print("{} --- Link: {} --- Identifier: {}".format(c,
              undownloaded_dict[key], key))
